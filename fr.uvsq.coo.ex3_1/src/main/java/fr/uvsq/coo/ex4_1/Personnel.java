...
public class Personnel implements Element {
	...
	public void insertPersonnel(String filename) {
		try (ObjectOutputStream out = new ObjectOutputStream(
			...
			out.writeObject(this.nomPersonnel);
			out.writeChars("|");
			out.writeObject(this.prenomPersonnel);
			out.writeChars("|");
			out.writeObject(this.dateNaissance);
			out.writeChars("|");
			for (NumeroTelephone numeroTelephone : numerosTelephone) {
				out.writeObject(numeroTelephone + ",");
			}
			out.writeChars("\n");
			out.close();
		} catch (Exception e) {
